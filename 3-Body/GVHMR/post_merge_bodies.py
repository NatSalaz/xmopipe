    ...
    try:
        ...
        for scene_key, body_key in expected_keys:
            if scene_key not in loaded or body_key not in loaded[scene_key]:
                print(f"Missing {scene_key}/{body_key} in merged file")
                return
    except Exception as e:
...
def process_all(input_root, npz_root, keep=False):
    ...
    for video_path in video_dirs:
        ...
        print(input_root, "tracking", f"tracking_{video_id}.json")
        tracking_path = os.path.join(input_root, "tracking", f"tracking_{video_id}.json")
        ...
        if set(video_files).issubset(processed_set):
            ...
            merge_body_npz(npz_path, keep=keep)
        else:
