...
def get_bbx_xys_from_xyxy(bbx_xyxy, base_enlarge=1.2):
    ...
    """
    # Vérification que l'entrée est un tenseur
    if not isinstance(bbx_xyxy, torch.Tensor):
        bbx_xyxy = torch.tensor(bbx_xyxy, dtype=torch.float32)

    # Vérification des dimensions
    if bbx_xyxy.ndim != 2 or bbx_xyxy.shape[1] != 4:
        raise ValueError(f"Expected input shape (N, 4), but got {bbx_xyxy.shape}")

    i_p2d = torch.stack([bbx_xyxy[:, [0, 1]], bbx_xyxy[:, [2, 3]]], dim=1)  # (N, 2, 2)
