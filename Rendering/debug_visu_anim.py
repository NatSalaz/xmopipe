...
"""Debugging application for HumanTOMATO npy files (T,322)"""
...
ap.add_argument("--npy2", type=str, default=None, help="Deuxième animation optionnelle")
ap.add_argument(
    "--model_path",
    type=str,
    required=False,
    default="/home/natsalaz/Documents/Baseline/OpenTMA-main/tomato_represenation/body_models/smplx",
)
...
# UTILS
# ------------------
def load_params(npy_path: str, start: int, end: int, device):
    params_np = np.load(npy_path)
    T_all = params_np.shape[0]
    start = max(0, start)
    end = T_all if end < 0 else min(end, T_all)
    params = torch.from_numpy(params_np[start:end]).to(
...
@torch.no_grad()
def smplx_vertices(frame: torch.Tensor) -> Tuple[np.ndarray, np.ndarray]:
    out = model(
        global_orient=frame[IDX["global_orient"]].view(1, 3),
        body_pose=frame[IDX["body_pose"]].view(1, -1),
        left_hand_pose=frame[IDX["left_hand_pose"]].view(1, -1),
        right_hand_pose=frame[IDX["right_hand_pose"]].view(1, -1),
        jaw_pose=frame[IDX["jaw_pose"]].view(1, 3),
        expression=frame[IDX["expression"]].view(1, -1),
        betas=frame[IDX["betas"]].view(1, -1),
        transl=frame[IDX["transl"]].view(1, 3),
    )
...
def checker_quads(size=10.0, tile=0.5, y=0.0):
    ...
    for iz in range(N):
        ...
        for ix in range(N):
            ...
            if (ix + iz) % 2 == 0:
                cols.extend([(0.2, 0.3, 0.2)] * 4)
            else:
                cols.extend([(0.1, 0.15, 0.1)] * 4)
            vid += 4
            x0 += step
        z0 += step
...
class Viewer:
    def __init__(self, animations):
        ...
        self.anim_index = 0
        self.params_name, self.params = self.animations[self.anim_index]
        self.T = self.params.shape[0]
        print("Precomputing vertices...")
        verts, joints = [], []
        for frame in self.params:
            V, J = smplx_vertices(frame)
            verts.append(V)
            joints.append(J)
        self.verts = verts
        self.joints = joints
        print(f"Done. {len(verts)} frames precomputed.")
        self.state = State()
        self.target_dt = 1.0 / max(1e-6, args.fps)

        V0, _ = self.verts[0], self.joints[0]
        ...
        self.app.initialize()
        self.window = self.app.create_window("SMPL-X Viewer", 1280, 800)
        ...
        self.material.base_color = (0.5, 0.75, 1.0, 1.0)
        self.mat_lines = rendering.MaterialRecord()
        self.mat_lines.shader = "unlitLine"
        self.mat_lines.line_width = 5.0
        self.scene.scene.add_geometry("smplx", self.mesh, self.material)

        self.ground = checker_quads(size=12.0, tile=0.5, y=self.y_ground)
        self.ground.compute_vertex_normals()
        sun_direction = np.array([-1.0, -1.0, -0.3], dtype=np.float32)
        self.scene.scene.show_skybox(True)
        self.scene.scene.set_background([0.0, 0.0, 0.0, 1.0])
        self.scene.scene.set_lighting(
            rendering.Open3DScene.LightingProfile.MED_SHADOWS, sun_direction
        )
        ...
        self.update_mesh_vertices(V0)
        _, J0 = smplx_vertices(self.params[0])
        self.update_geometry(V0, J0)
        self._last_mouse = None

    def create_skeleton(self, joints: np.ndarray) -> o3d.geometry.LineSet:
        edges = [
            (0, 1),
            (0, 2),
            (0, 3),
    ...
    def update_traj(self):
        ...
        for t in range(self.T):
            _, J = smplx_vertices(self.params[t])
    ...
    def seek(self, i: int):
        self.state.i = int(i)
        V, J = self.verts[i], self.joints[i]
        self.update_geometry(V, J)

    def update_geometry(self, V: np.ndarray, J: np.ndarray):
        if self.scene.scene.has_geometry("skeleton"):
            self.scene.scene.remove_geometry("skeleton")
        if self.scene.scene.has_geometry("smplx"):
            self.scene.scene.remove_geometry("smplx")

        if self.show_skeleton:
            if not hasattr(self, "skeleton"):
                self.skeleton = self.create_skeleton(J)
            else:
                self.skeleton.points = o3d.utility.Vector3dVector(J)
            self.scene.scene.add_geometry("skeleton", self.skeleton, self.mat_lines)
        else:
            self.mesh.vertices = o3d.utility.Vector3dVector(V.astype(np.float64))
            self.scene.scene.add_geometry("smplx", self.mesh, self.material)

        self.window.post_redraw()

    def update_mesh_vertices(self, V: np.ndarray):
        self.mesh.vertices = o3d.utility.Vector3dVector(V.astype(np.float64))
        self.mesh.compute_vertex_normals()
        self.scene.scene.remove_geometry("smplx")
        self.scene.scene.add_geometry("smplx", self.mesh, self.material)
    ...
    def on_toggle_skeleton(self, checked: bool):
        ...
        V, J = smplx_vertices(self.params[self.state.i])
        self.update_geometry(V, J)
    ...
    def on_switch_animation(self):
        ...
        print(f"Loading animation {self.params_name} ({self.T} frames)")
        verts, joints = [], []
        for frame in self.params:
            V, J = smplx_vertices(frame)
            verts.append(V)
        ...
        self.verts = verts
        self.joints = joints
        print(f"Animation {self.params_name} ready.")

        self.update_traj()
        self.label_file.text = f"Fichier: {self.params_name}"

        V0, J0 = self.verts[0], self.joints[0]
        self.update_geometry(V0, J0)
