...
class MaskGeneration(object):
    def __init__(
        ...
        self.perm = torch.zeros(
            (self.n_batch, max_len), device=device, dtype=torch.long
        )
        for i, length in enumerate(lengths):
            ...
            if permutation_mode == "random":
                ...
                key_points = 5
                self.perm[i, :length] = torch.floor(
                    (
                        torch.cos(
                            (2 * torch.pi * key_points / length)
                            * torch.linspace(0, length, length, device=device)
                        )
                        + 1
                    )
                    * (length - 1)
                ...
                self.perm[i, :length] = torch.cat(
                    (
                        torch.arange(length - 1, -1, -2, device=device),
                        torch.arange(0, length - 1, 2, device=device),
                    )
                )
            else:
                raise ValueError(
                    f"In MaskGeneration: invalid permutation mode {permutation_mode}"
                )
