...
def calculate_mpjpe(gt_joints, pred_joints):
    ...
    """
    assert (
        gt_joints.shape == pred_joints.shape
    ...
    # Compute MPJPE
    mpjpe = torch.linalg.norm(
        pred_joints - gt_joints, dim=-1
    )  # num_poses x num_joints=22
...
def calculate_R_precision_and_matscore(embedding1, embedding2, top_k, sum_all=False):
    dist_mat = euclidean_distance_matrix(embedding1, embedding2)
