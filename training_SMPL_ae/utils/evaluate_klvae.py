...
@torch.no_grad()
def evaluation_on_output(
    dataloader, net, output_file, logger: logging.Logger, eval_wrapper
):
    ...
    for k, (motion_name, output) in enumerate(outputs.items()):
        ...
        try:
            ...
            if len(output) == 0:
                ...
                index_motion = (
                    torch.tensor([[int(num) for num in output]]).cuda().long()
                )
        except:
            index_motion = torch.ones(1, seq).cuda().long()
        ...
        (
            ...
            token,
            name,
        ) = dataloader.dataset.get_data_by_name(motion_name)
