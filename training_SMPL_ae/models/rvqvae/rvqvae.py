...
class ResidualVQ(nn.Module):
        ...
        **kwargs
        ...
        if shared_codebook:
            ...
            self.layers = nn.ModuleList(
                [QuantizeEMAReset(**kwargs) for _ in range(num_quantizers)]
            )
    ...
    def forward(
        ...
        num_quant, quant_dropout_prob, device = (
            self.num_quantizers,
        ...
        should_quantize_dropout = (
            self.training and random.random() < self.quantize_dropout_prob
        )
        ...
        if should_quantize_dropout:
            start_drop_quantize_index = randrange(
        ...
        for quantizer_index, layer in enumerate(self.layers):
            ...
            # quantized, *rest = layer(residual, indices = layer_indices, sample_codebook_temp = sample_codebook_temp) #single quantizer
            quantized, *rest = layer(
                residual, return_idx=True, temperature=sample_codebook_temp
            )  # single quantizer
