...
def gumbel_sample(logits, temperature=1.0, stochastic=False, dim=-1, training=True):
    ...
    if training and stochastic and (temperature is not None and temperature > 0.0):
...
class QuantizeEMAReset(nn.Module):
    ...
    def forward(self, x, return_idx=False, temperature=0.0):
        ...
        commit_loss = F.mse_loss(
            x, x_d.detach()
        )  # It's right. the t2m-gpt paper is wrong on embed loss and commitment loss.
