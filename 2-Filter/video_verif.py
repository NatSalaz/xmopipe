...
def locked_file(file_path, mode="r+", max_retries=100, initial_delay=0.1):
    ...
    if not file_path.exists():
        with open(file_path, "w") as f:
            json.dump(
                {"processed": {}, "in_progress": {}, "failed": {}, "heartbeats": {}},
                f,
                indent=2,
            )
...
def load_tracking_data_safely(tracking_file):
    ...
    try:
        ...
        Log.error(
            f"Error loading tracking file {tracking_file}: {e}. Initializing new tracking data."
        )
        with open(tracking_file, "w") as f:
            data = {
                "processed": {},
...
class VideoTracker:
    ...
    def __init__(
        ...
        if not self.main_folder_path.is_dir():
            raise ValueError(
                f"Main folder path {self.main_folder_path} is not a directory"
            )
    ...
    def get_subfolders(self):
        return [
            f
            for f in self.main_folder_path.iterdir()
    ...
    def _get_tracking_file_for_subfolder(self, subfolder_path):
        ...
        if (
            self.main_folder_path not in subfolder_path.parents
            and subfolder_path != self.main_folder_path
        ):
            raise ValueError(
                f"Subfolder {subfolder_path} is not within main folder {self.main_folder_path}"
            )
    ...
    def is_video_failed(self, video_path):
        ...
        data = load_tracking_data_safely(tracking_file)
        video_path_str = str(Path(video_path).resolve())
        return video_path_str in data.get("failed", {})
    ...
    def is_subfolder_fully_processed(self, subfolder_path):
        ...
        for file_path in video_files:
            if (
                str(file_path) not in data["processed"]
                and str(file_path) not in data["failed"]
            ):
                return False
        return True
        ...
        try:
            with locked_file(tracking_file, "r+") as f:
                ...
                try:
                    ...
                    Log.warning(
                        f"Corrupted JSON detected in {tracking_file}, reinitializing."
                    )
                    data = {
                        "processed": {},
                        "in_progress": {},
                ...
                Log.info(f"Video marked as processed : {video_path_str}")

                # Checking folder
                subfolder_path = Path(video_path).parent
                if self.is_subfolder_fully_processed(subfolder_path):
                    Log.info(f"Complete folder : {subfolder_path}")
                    return True

                return False
        except Exception as e:
            Log.error(f"Error in marked video for {video_path_str} : {e}")
            return False
    ...
    def mark_video_failed(self, video_path, error_message=None):
        ...
        with locked_file(tracking_file, "r+") as f:
            ...
            Log.info(
                f"Video {video_path_str} marked as failed with error: {error_message}"
            )
    ...
    def mark_video_preprocessed(self, video_path):
        ...
        with locked_file(tracking_file, "r+") as f:
            ...
            Log.info(f"Video marked as preprocessed : {video_path_str}")
        ...
        for folder, folder_videos in videos_by_folder.items():
            ...
            with locked_file(tracking_file, "r+") as f:
                ...
                try:
                    ...
                    Log.warning(
                        f"Tracker file {tracking_file} corrupted in reservation. Reinitializing."
                    )
                    data = {
                        "processed": {},
                        "in_progress": {},
                ...
                for vid, info in data.get("in_progress", {}).items():
                    ...
                    if pid and str(pid) in data.get("heartbeats", {}):
                        last_heartbeat = data["heartbeats"][str(pid)].get(
                            "timestamp", 0
                        )
                        if (
                            ...
                            stale_keys.append(vid)
                            Log.warning(
                                f"Freeing reservation with stale heartbeat: {vid} (PID: {pid})"
                            )
                ...
                if local:
                    local_folder_reservations = sum(
                        1
                        for _, info in data["in_progress"].items()
                        if info.get("pid") == self._pid
                    )
                ...
                folder_available_slots = min(
                    folder_available_slots, available_slots - len(reserved)
                )
                ...
                folder_batch_size = min(
                    batch_size - len(reserved), folder_available_slots
                )
                ...
                available_videos = [
                    ...
                    if video not in data["processed"]
                    and video not in data["in_progress"]
                ]
                ...
                # Update heartbeat
                data.setdefault("heartbeats", {})
                folder_tracked_videos = [
                    v for v in self._tracked_videos if Path(v).parent == folder
                ]
                ...
                data["heartbeats"][str(self._pid)] = {
                    "timestamp": time.time(),
                    "videos": list(updated_videos),
                }

                # Save changes
                safe_write_json_atomic(tracking_file, data)

            # stops if we've reached our quota
            if len(reserved) >= batch_size or len(reserved) >= available_slots:
                break

        Log.info(
            f"Reserved {len(reserved)} videos across {len(videos_by_folder)} folders"
        )
        return reserved
        ...
        with locked_file(tracking_file, "r") as f:
            ...
            if video_path_str in data["in_progress"]:
                ...
                if elapsed > self.timeout_seconds or heartbeat_expired:
                    Log.warning(
                        f"Video {video_path_str} lock expired after {elapsed:.1f}s or heartbeat expired."
                    )
                    return True
                else:
                    Log.info(
                        f"Video {video_path_str} still in progress ({elapsed:.1f}s)."
                    )
                    return False
        ...
        while not self._stop_heartbeat.is_set():
            try:
                ...
                for folder, local_videos in videos_by_folder.items():
                    ...
                    with locked_file(
                        tracking_file, "r+", max_retries=100, initial_delay=0.1
                    ) as f:
                        f.seek(0)
                        try:
                            data = json.load(f)
                        ...
                        data["heartbeats"][heartbeat_pid] = {
                            ...
                            "videos": local_videos,  # only videos of this folder
                        }
                ...
                if videos_by_folder:
                    Log.debug(
                        f"Heartbeat updated for PID {self._pid} in {len(videos_by_folder)} folders"
                    )
        ...
        for tracking_file in tracking_files:
            try:
                with locked_file(tracking_file, "r+") as f:
                    ...
                    to_remove = [
                        key
                        for key in data.get("in_progress", {})
                ...
                Log.warning(
                    f"Failed to clean up processed videos in {tracking_file}: {e}"
                )
